...
public class ASTTest {
	...
	public void testProgram01() throws IllegalCharException, IllegalNumberException, SyntaxException {
		...
		ArrayList<Statement> st = b.getStatements();
		assertEquals(new ArrayList<Statement>(), st);
	...
	@Test
	public void testOffical01() throws IllegalCharException, IllegalNumberException, SyntaxException {
		String input = "prog0 {}";
		Scanner scanner = new Scanner(input);
		scanner.scan();
		Parser parser = new Parser(scanner);
		...
		assertEquals(IDENT, prog.getFirstToken().kind);
		assertEquals(0,prog.getFirstToken().getLinePos().line);
		assertEquals(0,prog.getFirstToken().getLinePos().posInLine);
		assertEquals(java.util.ArrayList.class, prog.getParams().getClass());
		assertEquals(0,prog.getParams().size());
		assertEquals(Block.class, prog.getB().getClass());
		Block b = prog.getB();
		assertEquals("{", b.getFirstToken().getText());
		assertEquals(6,b.getFirstToken().pos);
		assertEquals(0,b.getDecs().size());
		assertEquals(0,b.getStatements().size());
	}
	
	@Test
	public void testOfficial02() throws IllegalCharException, IllegalNumberException, SyntaxException {
		String input = "abc\n{integer x\nif(true){integer x}\n}";
		Scanner scanner = new Scanner(input);
		scanner.scan();
		Parser parser = new Parser(scanner);
		...
		Program be = (Program) ast;
		assertEquals(IDENT, be.getFirstToken().kind);
		assertEquals(0, be.getFirstToken().getLinePos().posInLine);
		assertEquals(java.util.ArrayList.class, be.getParams().getClass());
		assertEquals(Block.class, be.getB().getClass());
		Block b = be.getB();
		assertEquals("{", b.getFirstToken().getText());
		Dec a = b.getDecs().get(0);
		assertEquals(KW_INTEGER, a.getFirstToken().kind);
		assertEquals(1, a.getFirstToken().getLinePos().posInLine);
		assertEquals("x", a.getIdent().getText());
		IfStatement ifs = (IfStatement)b.getStatements().get(0);
		assertEquals(KW_IF, ifs.getFirstToken().kind);
		assertEquals(0, ifs.getFirstToken().getLinePos().posInLine);
		Block ifBlock = ifs.getB();
		assertEquals(KW_INTEGER, ifBlock.getDecs().get(0).getFirstToken().kind);
	}
	
	@Test
	public void testOfficial03() throws IllegalCharException, IllegalNumberException, SyntaxException {
		String input = "abc \n{x \n<- \n33;}";
		Scanner scanner = new Scanner(input);
		scanner.scan();
		Parser parser = new Parser(scanner);
		...
		Program be = (Program) ast;
		assertEquals(IDENT, be.getFirstToken().kind);
		assertEquals(0, be.getFirstToken().pos);
		assertEquals("abc", be.getFirstToken().getText());
		assertEquals(java.util.ArrayList.class, be.getParams().getClass());
		assertEquals(Block.class, be.getB().getClass());
		Block b = be.getB();
		assertEquals("{", b.getFirstToken().getText());
		AssignmentStatement a = (AssignmentStatement)b.getStatements().get(0);
		assertEquals(IDENT, a.getFirstToken().kind);
		assertEquals(1, a.getFirstToken().getLinePos().line);
	...
	public void testFactor4() throws IllegalCharException, IllegalNumberException, SyntaxException {
		String input = "screenheight 1234"; // Note that 1234 will not be parsed
	...
	public void testElem1() throws IllegalCharException, IllegalNumberException, SyntaxException {
		String input = "(abc*1234%5/def&&true)"; // strongOp not followed by
													// factor
		Scanner scanner = new Scanner(input);
		scanner.scan();
		Parser parser = new Parser(scanner);
	...
	public void testTerm1() throws IllegalCharException, IllegalNumberException, SyntaxException {
		...
		scanner.scan();
		Parser parser = new Parser(scanner);
		ASTNode ast = parser.term(); // (abc*1234%5/def&true, |,
										// false&21)+121-something
		assertEquals(BinaryExpression.class, ast.getClass());
		BinaryExpression binExp = (BinaryExpression) ast;
	...
	public void testTerm3() throws IllegalCharException, IllegalNumberException, SyntaxException {
		...
		Parser parser = new Parser(scanner);
		ASTNode ast = parser.term(); // abc*1234%5/def&true, |,
										// false&21+121-something // will stop
										// at
										// abc*1234%5/def&true|false&21+121-something
		assertEquals(BinaryExpression.class, ast.getClass());
		BinaryExpression binExp = (BinaryExpression) ast;
		assertEquals(MINUS, binExp.getOp().kind);
		assertEquals(BinaryExpression.class, binExp.getE0().getClass());
	...
	@Test
	public void test0() throws IllegalCharException, IllegalNumberException, SyntaxException {
	...
	public void test4() throws IllegalCharException, IllegalNumberException, SyntaxException {
		...
		assertEquals(IntLitExpression.class, tu.getExprList().get(0).getClass());
		assertEquals(IdentExpression.class, tu.getExprList().get(1).getClass());
	}
